...
.. code-block:: javascript
   ...
   class LocalVectorDBClient {
       ...
       async addDocuments(dbName, documents, metadata = null) {
           ...
           return this.makeRequest(`/api/v1/${dbName}/documents/upsert`, {
               method: 'POST',
               headers: { 'Content-Type': 'application/json' },
       ...
       async search(dbName, query, options = {}) {
           ...
           return this.makeRequest(`/api/v1/${dbName}/query?${params}`);
       }
   ...
   async function examples() {
       const client = new LocalVectorDBClient('http://localhost:5000', 'your-api-key');
       ...
       try {
           ...
           // Add text documents
           const result = await client.addDocuments('my_docs',
               ['This is a sample document about machine learning.'],
               [{ category: 'tutorial', tags: ['ml', 'ai'] }]
           );
           console.log('Documents added:', result.ids);

           // Search documents
           const searchResults = await client.search('my_docs', 'machine learning', {
               searchType: 'hybrid',
               limit: 5,
           ...
           // Upload files (if you have file input)
           const fileInput = document.getElementById('fileInput');
           if (fileInput && fileInput.files.length > 0) {
               const uploadResult = await client.uploadFiles('my_docs', fileInput.files, {
                   useFilenameAsId: true,
                   metadata: [{ source: 'web_upload', uploaded_by: 'user' }]
               });
               console.log('Files uploaded:', uploadResult);
           }

           // Get database stats
           const stats = await client.getStats('my_docs');

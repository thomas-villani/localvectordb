...
# ``QueryBuilderInterface`` (the portable fluent contract shared by the local and
# remote query builders) lives in the dependency-light ``localvectordb._interfaces``
# module so that ``database.base`` can reference it for typing without importing
# ``query_builder`` (which would form a ``base`` <-> ``query_builder`` cycle). It is
# re-exported here for backwards compatibility -- ``from localvectordb.query_builder
# import QueryBuilderInterface`` keeps working.

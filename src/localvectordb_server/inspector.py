...
inspector_bp = Blueprint(
    "inspector", __name__, template_folder="templates", static_folder="static", static_url_path="/inspector/static"
)
            ...
            if config and config.server.security.require_api_key:
                ...
                try:
                    ...
                    if key_manager:
                        ...
                        if (
                            required_permission == PermissionLevel.READ_WRITE
    ...
    try:
        ...
        for db_name in databases:
            try:
                ...
                db_stats.append({"name": db_name, "error": str(e), "documents": 0, "chunks": 0})

        # Get system information
        manager_stats = db_manager.get_manager_stats()

        return render_template(
            "dashboard.html", databases=db_stats, system_stats=manager_stats, total_databases=len(databases)
        )
...
def database_detail(db_name):
    ...
    try:
        ...
        return render_template(
            "database.html", db_name=db_name, stats=stats, schema_info=schema_info, config=config_info
        )
...
def admin_interface():
    ...
    try:
        ...
        return render_template("admin.html", manager_stats=manager_stats, config=config, api_keys=api_keys_info)
...
def inspector_not_found(e):
    ...
    return (
        render_template("error.html", error="Page not found", message="The requested inspector page was not found."),
        404,
    )
...
def inspector_server_error(e):
    ...
    return (
        render_template(
            "error.html", error="Server error", message="An internal server error occurred in the inspector."
        ),
        500,
    )

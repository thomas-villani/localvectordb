...
def upsert_documents(db_name):
    ...
    with request_context("upsert_documents"):
        ...
        try:
            ...
            db_logger.log_query("upsert_documents_success", database_name=db_name, result_count=len(result_ids))
...
def upsert_from_chunks(db_name):
    ...
    with request_context("upsert_from_chunks"):
        ...
        # Convert incoming dict chunks to simple strings and let LocalVectorDB normalize to Chunk objects
        processed_chunks = {}
        ...
        try:
            ...
            db_logger.log_query("upsert_from_chunks", database_name=db_name, document_count=len(chunks_by_document))
...
def insert_from_chunks(db_name):
    ...
    with request_context("insert_from_chunks"):
        ...
        try:
            ...
            db_logger.log_query("insert_from_chunks", database_name=db_name, document_count=len(chunks_by_document))
...
def update_document(db_name, doc_id):
    ...
    with request_context("update_document"):
        ...
        try:
            ...
            return jsonify(
                {"message": f"Successfully updated document {doc_id}", "status": "success", "updated": was_updated}
            )
...
def delete_documents_batch(db_name):
    ...
    with request_context("delete_documents_batch"):
        try:
            ...
            if not ids:
                return jsonify(
                    {"message": "No documents to delete", "status": "success", "deleted_count": 0, "failed_ids": []}
                )
...
def check_documents_exist(db_name):
    ...
    with request_context("check_documents_exist"):
        ...
        try:
            ...
            return jsonify({"exists": exists, "ids": ids})
    ...
    with request_context("list_documents" if not ids else "get_documents"):
        try:
            ...
            if ids:
                ...
                return jsonify({"documents": serialized_docs, "returned_ids": returned_ids, "missing_ids": missing_ids})

            # Validate pagination parameters
            page, limit = validate_pagination_params(request.args.get("page"), request.args.get("limit"))
        ...
        try:
            ...
            for clause in search_clauses:
                builder = builder.search(
                    clause["text"], columns=clause.get("columns"), search_type=clause.get("search_type", "hybrid")
                )
...
def global_search():
    ...
    with request_context("global_search"):
        ...
        try:
            ...
            return jsonify({"results": results, "search_type": search_type, "return_type": return_type})
...
def get_embeddings():
    ...
    with request_context("get_embeddings"):
        ...
        if provider not in EmbeddingRegistry.list():
            raise ValidationError(
                f"Provider must be one of: {', '.join(EmbeddingRegistry.list())}", field="provider", value=provider
            )
...
def upload_files(db_name):
    ...
    with request_context("upload_files"):
        ...
        file_upload_enabled = (
            current_app.config_obj.server.file_upload_enabled if hasattr(current_app, "config_obj") else True
        )
        if not file_upload_enabled:
            raise APIError(
                message="File extraction route is not enabled", error_code="EXTRACTION_NOT_AVAILABLE", status_code=503
            )
        ...
        if file_ids is not None:
            if len(file_ids) != len(files):
                raise ValidationError(
                    f"Number of IDs ({len(file_ids)}) must match number of files ({len(files)})", field="ids"
                )
...
def extract_preview():
    ...
    with request_context("extract_preview"):
        ...
        if not file_upload_enabled:
            raise APIError(
                message="File extraction is not enabled", error_code="EXTRACTION_NOT_AVAILABLE", status_code=503
            )
        ...
        try:
            ...
            raise APIError(
                message=f"Preview extraction failed: {str(e)}", error_code="EXTRACTION_PREVIEW_FAILED", status_code=500
            ) from e
...
def get_metadata_schema_info(db_name):
    ...
    with request_context("get_metadata_schema_info"):
        try:
            ...
            return jsonify({"database": db_name, "schema_info": schema_info, "status": "success"})
...
def get_sqlite_tuning(db_name):
    ...
    with request_context("get_sqlite_tuning"):
        try:
            ...
            return jsonify({"database": db_name, "tuning": tuning_config, "status": "success"})
...
def set_sqlite_tuning(db_name):
    ...
    with request_context("set_sqlite_tuning"):
        try:
            ...
            db_logger.log_query("set_sqlite_tuning_success", database_name=db_name, profile=profile)
...
def sqlite_checkpoint(db_name):
    ...
    with request_context("sqlite_checkpoint"):
        try:
            ...
            db_logger = DatabaseLogger.get_logger(db_name)
            db_logger.log_query("sqlite_checkpoint", database_name=db_name, mode=mode)

            # Run checkpoint
            db.sqlite_checkpoint(mode)

            db_logger.log_query("sqlite_checkpoint_success", database_name=db_name, mode=mode)
...
def sqlite_optimize(db_name):
    ...
    with request_context("sqlite_optimize"):
        try:
            ...
            return jsonify({"database": db_name, "message": "SQLite PRAGMA optimize completed", "status": "success"})
...
def sqlite_incremental_vacuum(db_name):
    ...
    with request_context("sqlite_incremental_vacuum"):
        try:
            ...
            db_logger = DatabaseLogger.get_logger(db_name)
            db_logger.log_query("sqlite_incremental_vacuum", database_name=db_name, pages=pages)

            # Run incremental vacuum
            db.sqlite_incremental_vacuum(pages)

            db_logger.log_query("sqlite_incremental_vacuum_success", database_name=db_name, pages=pages)
...
def analyze_system_resources():
    ...
    with request_context("analyze_system_resources"):
        try:
            ...
            db_logger.log_query(
                "analyze_system_resources_success",
                total_ram_mb=system_info.total_ram_mb,
                disk_type=system_info.disk_type,
            )

            return jsonify({"system_resources": resources, "status": "success"})
...
def auto_tune_database(db_name):
    ...
    with request_context("auto_tune_database"):
        try:
            ...
            return jsonify({"database": db_name, "recommendation": recommendation, "status": "success"})
...
def checkpoint_if_wal_large(db_name):
    ...
    with request_context("checkpoint_if_wal_large"):
        try:
            ...
            db_logger = DatabaseLogger.get_logger(db_name)
            db_logger.log_query("checkpoint_if_wal_large", database_name=db_name, threshold_mb=threshold_mb)

            # Check and checkpoint if needed
            checkpointed = db.checkpoint_if_wal_large(threshold_mb)

            db_logger.log_query("checkpoint_if_wal_large_success", database_name=db_name, checkpointed=checkpointed)

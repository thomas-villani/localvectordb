...
class ValidationError(APIError):
    ...
    def __init__(self, message: str, field: str = None, value: Any = None, **kwargs):
        ...
        super().__init__(
            message=message, error_code="VALIDATION_ERROR", status_code=400, details=details, recoverable=True, **kwargs
        )


def standardize_error_response(
    error: Exception, default_message: str = "An unexpected error occurred", default_code: str = "INTERNAL_ERROR"
    ...
    if semantic_filters is not None:
        ...
        for idx, filter_item in enumerate(semantic_filters):
            if not isinstance(filter_item, dict):
                raise ValidationError(
                    f"semantic_filters[{idx}] must be a dictionary", field=f"semantic_filters[{idx}]", value=filter_item
                )

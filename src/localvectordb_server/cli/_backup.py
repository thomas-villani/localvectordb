...
from localvectordb.backup import (
    BackupConfig, BackupManager, BackupType, CompressionAlgorithm,
    IncrementalBackupManager, PointInTimeRecoveryManager
)
...
def restore_backup(ctx, backup_id, to_location, overwrite, location, output_json):
    ...
    try:
        ...
        if output_json:
            ...
            click.secho(f"✓ Restore completed successfully", fg="green")
...
def verify_backup(backup_id, location, output_json):
    ...
    try:
        ...
        if output_json:
            ...
            if is_valid:
                click.secho(f"✓ Backup verification passed", fg="green")
            else:
                click.secho(f"✗ Backup verification failed", fg="red")
    ...
    try:
        ...
        if output_json:
            ...
            if recovery_result['success']:
                if dry_run:
                    click.secho(f"✓ Point-in-time recovery validation passed", fg="green")
                else:
                    click.secho(f"✓ Point-in-time recovery completed", fg="green")
                ...
                click.secho(f"✗ Point-in-time recovery failed", fg="red")

...
def migration_status(ctx, database_name, migrations_dir, output_format):
    ...
    try:
        ...
        migration_engine = MigrationEngine(database_path=db_path, migrations_directory=migrations_dir)
...
@click.option("--backup/--no-backup", default=True, help="Create backup before migration (default: enabled)")
@click.option(
    "--backup-location",
    "-b",
...
def apply_migrations(ctx, database_name, to_version, migrations_dir, backup, backup_location, dry_run, output_format):
    ...
    try:
        ...
        # Apply migrations
        result = migration_engine.migrate(target_version=to_version, dry_run=dry_run, create_backup=backup)
...
@click.option("--backup/--no-backup", default=True, help="Create backup before rollback (default: enabled)")
@click.option(
    "--backup-location",
    "-b",
    type=click.Path(exists=False, file_okay=False),
...
def rollback_migrations(
    ...
    try:
        ...
        # Perform rollback
        result = migration_engine.rollback(target_version=target_version, dry_run=dry_run, create_backup=backup)
...
def create_migration(description, version, migrations_dir, template, output_format):
    ...
    try:
        ...
        migration_file = migration_engine.create_migration_template(
            version=version, description=description, template_type=template
        )
        ...
        result = {"success": False, "error": str(e), "version": version, "description": description}

...
class TestUtilityFunctions:
    ...
    def test_find_chunk_containing_position(self):
        """Test finding chunk containing a position."""
        chunks = [
            Chunk(
                content="Hello ",
                position=ChunkPosition(start=0, end=6, line=1, column=1, end_line=1, end_column=6),
        ...
        # Position in first chunk
        chunk = find_chunk_containing_position(chunks, 3)
        assert chunk is not None
        assert chunk.index == 0

        # Position in second chunk
        chunk = find_chunk_containing_position(chunks, 8)
        assert chunk is not None
        assert chunk.index == 1

        # Position at boundary
        chunk = find_chunk_containing_position(chunks, 6)
        assert chunk is not None
        assert chunk.index == 1  # Should find second chunk

        # Position outside range
        chunk = find_chunk_containing_position(chunks, 15)
        assert chunk is None

    def test_get_chunks_in_range(self):
        """Test getting chunks that overlap with a range."""
        chunks = [
            Chunk(
                content="Hello ",
                position=ChunkPosition(start=0, end=6, line=1, column=1, end_line=1, end_column=6),
            ...
            Chunk(
                content="world ",
                position=ChunkPosition(start=6, end=12, line=1, column=7, end_line=1, end_column=12),
                tokens=1,
                index=1
            ),
            Chunk(
                content="test",
                position=ChunkPosition(start=12, end=16, line=1, column=13, end_line=1, end_column=16),
                tokens=1,
                index=2
            )
        ]

        # Range overlapping first two chunks
        overlapping = get_chunks_in_range(chunks, 3, 10)
        assert len(overlapping) == 2
        assert overlapping[0].index == 0
        assert overlapping[1].index == 1

        # Range covering all chunks
        overlapping = get_chunks_in_range(chunks, 0, 16)
        assert len(overlapping) == 3

        # Range with no overlap
        overlapping = get_chunks_in_range(chunks, 20, 25)
        assert len(overlapping) == 0

        # Range overlapping single chunk
        overlapping = get_chunks_in_range(chunks, 13, 15)
        assert len(overlapping) == 1
        assert overlapping[0].index == 2